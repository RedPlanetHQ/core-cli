...
// Function to update lastTaskSync timestamp in config file
export function updateLastTaskSync(timestamp: string): void {
	const configJsonPath = getClosestConfigFile('config.json');
	...
	try {
		...
		configData.core.lastTaskSync = timestamp;
		...
		console.warn('Failed to update lastTaskSync:', error);
	}
}
