...
import {initializeSession, getCurrentSession} from '@/usage/tracker';
...
export function createClearMessagesHandler(
	...
	return async () => {
		...
		if (currentSession) {
			...
			// Reinitialize session with current provider and model (creates new session ID)
			initializeSession(sessionInfo.provider, sessionInfo.model);
		}
	};
}
