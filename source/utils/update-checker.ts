...
const UPDATE_COMMANDS = {
	NPM: 'npm update -g @redplanethq/core',
	// Check if package exists before upgrading to provide better error messages
	HOMEBREW:
		'brew list core >/dev/null 2>&1 && brew upgrade core || (echo "Error: core not found in Homebrew. Please install it first with: brew install core" && exit 1)',
} as const;
...
async function fetchLatestVersion(): Promise<string | null> {
	try {
		const response = await fetch(
			'https://registry.npmjs.org/@redplanethq/core-cli/latest',
			{
				method: 'GET',
				headers: {
					Accept: 'application/json',
					'User-Agent': 'core-update-checker',
				},
				// Add timeout
